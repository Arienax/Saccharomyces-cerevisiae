...
clear_img = {0: 'crash', 1: 'crash', 2: 'comp', 3: 'comp_ex', 4: 'comp_ultra', 5: 'uc', 6: 'puc', 7: 'saved'}
clear_table = {0: 'FAILED', 1: 'FAILED', 2: 'NC', 3: 'HC', 4: 'EX-HC', 5: 'UC', 6: 'PUC', 7: 'SAVED'}
clear_palette = ('#FFFFFF', '#32936F', '#69A297', '#A5668B', '#E83F6F', '#FFBF00')
clear_legend = ('N/A', 'CRASH/SAVE', 'NC', 'HC/EX-HC', 'UC', 'PUC')
...
def get_bpm_str(bpm_max: str, bpm_min: str) -> str:
    if bpm_max[-2:] == '00':
        bpm_max = bpm_max[:-2]
    else:
    ...
    if refactor:
        for index in range(8):
            if clear_list[index] is not None:
                clear_list[index] = cv2.resize(clear_list[index], dsize=None, fx=refactor, fy=refactor, interpolation=cv2.INTER_AREA)
    return clear_list
