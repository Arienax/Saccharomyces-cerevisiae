...
fake_table = [[''] * 26 for _ in range(max_id + 1)]
for i in range(len(fake_table)):
    fake_table[i][5] = fake_table[i][6] = '0'
    fake_table[i][9] = '2'
    fake_table[i][10] = fake_table[i][13] = fake_table[i][16] = fake_table[i][19] = fake_table[i][22] = '0'

for _mid, _song in db_reader.music_db.items():
    row = fake_table[_mid]
    row[0] = str(_mid)
    row[1] = str(_song.get('info', {}).get('title_name', 'Unknown'))
    row[3] = str(_song.get('info', {}).get('artist_name', 'Unknown'))
    row[9] = str(_song.get('info', {}).get('inf_ver', '2'))
    diffs = _song.get('difficulty', {})
    row[10] = str(int(float(diffs.get('novice', 0))))
    row[13] = str(int(float(diffs.get('advanced', 0))))
    row[16] = str(int(float(diffs.get('exhaust', 0))))
    row[19] = str(int(float(diffs.get('infinite', 0))))
...
<call model="KFC:J:G:A:2026011300" srcid="00010203040506070809" tag="test">
    <game method="sv7_load_m" ver="0">
        ...
        try:
            ...
            # 强制解码文本并用正则提取，彻底绕过 XML 解析器的编码报错
            text_content = resp.content.decode('shift_jis', errors='ignore')
            param_list = re.findall(r'<param\s+[^>]*>(.*?)</param>', text_content)
            
            for text in param_list:
                ...
                record = [True, mid, m_type, score, clear, grade, 0, ex_score, 0, 0, 0]
                self.music_map.append(record)
...
def direct_get_jacket(mid, m_type, size='b'):
    jk = db_reader.get_jacket_image(mid, m_type)
    
    # SDVX 很多老歌的高难度（如 XCD、MXM）没有独立封面，默认与 EXH 共用。
    # 查分器底层若找不到文件，会错误地回落并返回 NOV(0) 的封面。
    # 拦截逻辑：如果是高级谱面，且拿到的图和 NOV 居然一模一样（或者为空），就强制向下寻找专属封面
    if m_type >= 3:
        jk_nov = db_reader.get_jacket_image(mid, 0)
        is_fallback_to_nov = (jk is not None and jk_nov is not None and np.array_equal(jk, jk_nov))
        
        if jk is None or is_fallback_to_nov:
            # 往下级寻找（比如先找 EXH），找到第一个和 NOV 长得不一样的专属图就停下
            for fallback_type in range(m_type - 1, 0, -1):
                jk_fallback = db_reader.get_jacket_image(mid, fallback_type)
                if jk_fallback is not None and (jk_nov is None or not np.array_equal(jk_fallback, jk_nov)):
                    jk = jk_fallback
                    break
...
def plot_b50(_music_map: list = None, profile: list = None) -> str:
    ...
    try:  # If the plot module breaks down somehow, the function will try to return the pure text data.
        ...
        level_ref, clear_ref, grade_ref = 0, 0.4, 0.42
        ...
        for index in range(50):
            ...
            score_color = [color_white for _ in range(8)]  # Let foremost '0's be gray
...
def plot_summary(base_lv: int, _music_map: list = None, profile: list = None):
    ...
    clear_index = {0: 0, 1: 1, 2: 2, 3: 3, 4: 3, 5: 4, 6: 5, 7: 1}
    ...
    try:  # If the plot module breaks down somehow, the function will try to return the pure text data.
        ...
        # Load image ingredients
        clear_ref, grade_ref, box_ref = 1.09, 0.45, 1.09
